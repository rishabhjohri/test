...
--------------------------------------------------------------------------------
-- Migrate notice text columns from VARCHAR2 to CLOB.
-- Oracle does not support ALTER TABLE ... MODIFY VARCHAR2_COLUMN CLOB directly.
-- The migration preserves existing values by copying them into temporary CLOB
-- columns before dropping and renaming the original columns.
--------------------------------------------------------------------------------

ALTER TABLE IE_APP_ONLINE.CO_MASS_MAILING_REQ
    ADD NOTICE_TXT_CLOB_TMP CLOB;

ALTER TABLE IE_APP_ONLINE.CO_MASS_MAILING_REQ
    ADD NOTICE_ES_CLOB_TMP CLOB;

ALTER TABLE IE_APP_ONLINE.CO_MASS_MAILING_REQ
    ADD NOTICE_PT_CLOB_TMP CLOB;

UPDATE IE_APP_ONLINE.CO_MASS_MAILING_REQ
   SET NOTICE_TXT_CLOB_TMP = TO_CLOB(NOTICE_TXT),
       NOTICE_ES_CLOB_TMP  = TO_CLOB(NOTICE_TEXT_ES),
       NOTICE_PT_CLOB_TMP  = TO_CLOB(NOTICE_TEXT_PT);

COMMIT;

ALTER TABLE IE_APP_ONLINE.CO_MASS_MAILING_REQ
    DROP COLUMN NOTICE_TXT;

ALTER TABLE IE_APP_ONLINE.CO_MASS_MAILING_REQ
    DROP COLUMN NOTICE_TEXT_ES;

ALTER TABLE IE_APP_ONLINE.CO_MASS_MAILING_REQ
    DROP COLUMN NOTICE_TEXT_PT;

ALTER TABLE IE_APP_ONLINE.CO_MASS_MAILING_REQ
    RENAME COLUMN NOTICE_TXT_CLOB_TMP TO NOTICE_TXT;

ALTER TABLE IE_APP_ONLINE.CO_MASS_MAILING_REQ
    RENAME COLUMN NOTICE_ES_CLOB_TMP TO NOTICE_TEXT_ES;

ALTER TABLE IE_APP_ONLINE.CO_MASS_MAILING_REQ
    RENAME COLUMN NOTICE_PT_CLOB_TMP TO NOTICE_TEXT_PT;
